...
class IncomingEventProcessor(BaseIncomingEventProcessor):
    ...
    async def _handle_message(self, event: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            initial_event_details = {"message": message, "attachments": attachments, "server": server}
            user_info_preprocessor = UserInfoPreprocessor(self.config, self.client)
            initial_event_details.update(await user_info_preprocessor.process_incoming_event(message))
    ...
    async def _handle_message_change(self, event: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        initial_event_details = {"event_type": "update_message", "message": event, "attachments": attachments}
        user_info_preprocessor = UserInfoPreprocessor(self.config, self.client)
    ...
    async def _handle_delete_message(self, event: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            delta = await self.conversation_manager.delete_from_conversation(incoming_event=event)

            if delta:
                for deleted_id in delta.get("deleted_message_ids", []):
                    events.append(self.incoming_event_builder.message_deleted(deleted_id, delta["conversation_id"]))
        except Exception as e:
        ...
        try:
            ...
            if delta:
                for reaction in delta.get("added_reactions", []):
                    events.append(self.incoming_event_builder.reaction_update("reaction_added", delta, reaction))
                for reaction in delta.get("removed_reactions", []):
                    events.append(self.incoming_event_builder.reaction_update("reaction_removed", delta, reaction))
        except Exception as e:
