...
from src.adapters.zulip_adapter.event_processing.user_info_preprocessor import UserInfoPreprocessor
...
class OutgoingEventProcessor(BaseOutgoingEventProcessor):
    ...
    async def _send_message(self, conversation_info: Any, data: BaseModel) -> Dict[str, Any]:
        ...
        messages = self._split_long_message(await user_info_preprocessor.process_outgoing_event(data.mentions, data.text))
        ...
        if conversation_info.conversation_type == "stream":
            to_field = conversation_info.stream_name
            subject = conversation_info.stream_topic
        else:
            to_field = conversation_info.emails()
    ...
    async def _edit_message(self, conversation_info: Any, data: BaseModel) -> Dict[str, Any]:
        ...
        """
        await self.rate_limiter.limit_request("update_message", conversation_info.conversation_id)

        user_info_preprocessor = UserInfoPreprocessor(self.config, self.client)
        message_data = {
            "message_id": int(data.message_id),
            "content": await user_info_preprocessor.process_outgoing_event(data.mentions, data.text)
        }
    ...
    async def _delete_message(self, conversation_info: Any, data: BaseModel) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Dictionary containing the status
        """
        await self.rate_limiter.limit_request("delete_message", conversation_info.conversation_id)
    ...
    async def _add_reaction(self, conversation_info: Any, data: BaseModel) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Dictionary containing the status
        """
        await self.rate_limiter.limit_request("add_reaction", conversation_info.conversation_id)
    ...
    async def _remove_reaction(self, conversation_info: Any, data: BaseModel) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Dictionary containing the status
        """
        await self.rate_limiter.limit_request("remove_reaction", conversation_info.conversation_id)
